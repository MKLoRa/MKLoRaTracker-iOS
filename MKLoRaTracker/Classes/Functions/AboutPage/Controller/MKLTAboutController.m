...
@implementation MKLTAboutController

- (void)dealloc {
    NSLog(@"MKLTAboutController销毁");
}
...
- (UIImageView *)aboutIcon{
    if (!_aboutIcon) {
        ...
        _aboutIcon.image = LOADICON(@"MKLoRaTracker", @"MKLTAboutController", @"lt_lw004CT-aboutIcon.png");;
    }
    return _aboutIcon;
}
...
- (UILabel *)firmwareLabel{
    if (!_firmwareLabel) {
        ...
        _firmwareLabel.text = @"FW Version:2.1";
    }
...
- (UIImageView *)bottomIcon {
    if (!_bottomIcon) {
        ...
        _bottomIcon.image = LOADICON(@"MKLoRaTracker", @"MKLTAboutController", @"lt_aboutBottomIcon.png");
    }
    return _bottomIcon;
}
