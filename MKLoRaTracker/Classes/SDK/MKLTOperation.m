...
NSString *const mk_lt_additionalInformation = @"mk_lt_additionalInformation";
NSString *const mk_lt_dataInformation = @"mk_lt_dataInformation";
NSString *const mk_lt_dataStatusLev = @"mk_lt_dataStatusLev";

@interface MKLTOperation ()
...
/**
 线程ID
 */
@property (nonatomic, assign)mk_lt_taskOperationID operationID;
...
/**
 线程结束时候的回调
 */
@property (nonatomic, copy)void (^completeBlock) (NSError *error, mk_lt_taskOperationID operationID, id returnData);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKLTTaskAdopter parseReadDataWithCharacteristic:characteristic]];
}

- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic {
    [self dataParserReceivedData:[MKLTTaskAdopter parseWriteDataWithCharacteristic:characteristic]];
}
    ...
    if (self.completeBlock) {
        if (self.needPartOfData) {
            ...
            NSDictionary *resultDic = @{
                                        mk_lt_additionalInformation:(self.additionalInformation ?: @{}),
                                        mk_lt_dataInformation:self.dataList,
                                        //对于有附加信息的，lev为2，对于普通不包含附加信息的，lev为1.
                                        mk_lt_dataStatusLev:(self.additionalInformation ? @"2" : @"1"),
                                        };
            if (self.completeBlock) {
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_lt_taskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_lt_defaultTaskOperationID || operationID != self.operationID) {
        return;
    }
    ...
    NSString *numString = returnData[mk_lt_communicationDataNum];
